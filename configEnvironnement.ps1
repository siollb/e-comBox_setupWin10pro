...
If ($TestPath -eq $False) {
    ...
    #Write-Host ""
      ...
      docker stop $(docker ps -q) *>> $pathlog\configEnvEcombox.log
      If (docker ps -a --filter "name=portainer-app") {
         docker rm -f portainer-app *>> $pathlog\configEnvEcombox.log
         }
      If (docker ps -a --filter "name=portainer-proxy") {
         docker rm -f portainer-proxy *>> $pathlog\configEnvEcombox.log
         }
      If (docker ps -a --format '{{.Names}}' |  Select-String portainer-proxy) {
         docker rm -f $(docker ps -a --format '{{.Names}}' |  Select-String portainer-proxy)  *>> $pathlog\configEnvEcombox.log
         }
      Set-Location -Path $env:USERPROFILE\
...
# Démarrage de Portainer
   #docker-compose down *>> $pathlog\configEnvEcombox.log
   #docker stop $(docker ps -a -q) *>> $pathlog\configEnvEcombox.log
   docker-compose up --build -d *>> $pathlog\configEnvEcombox.log
...
if ((docker ps |  Select-String portainer-proxy)) {
    ...
    write-host ""
    write-Output "" >> $pathlog\configEnvEcombox.log
    write-Output "Portainer est UP, on peut continuer" >> $pathlog\configEnvEcombox.log
    write-Output "" >> $pathlog\configEnvEcombox.log
    }
      else {
            write-host ""
            ...
            Write-Host ""
            Write-Output "" >> $pathlog\configEnvEcombox.log
            Write-Output "Toutes les tentatives pour démarrer Portainer ont échoué" >> $pathlog\configEnvEcombox.log
...
if (docker image ls -q) {
  ...
  docker image rm $(docker image ls -q) *>> $pathlog\configEnvEcombox.log
}
  else {
       Write-host ""
